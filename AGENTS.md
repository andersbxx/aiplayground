...
- **Export:** varje session kan laddas ner som Markdown (`exportSession` / `buildExportMd`) med prompts + demos som ```html-block, för att mata vidare i en annan AI. Demo-korten har också nedladdning av enstaka `.html` (data-act="dl", blob + a.download).
- **Import:** filvalknappen "Importera .md" (skjulen `input[type=file]`) + drag & drop (≥900px-desktop) återanvänder `parseImportedMarkdown` (tar emot exakt det format `buildExportMd` skapar; stödjer `## Prompt N — Du`, `## AI-svar (planering)` och `## Demo N`-sektioner; oläkänliga filer avvisas med toast) och `addImportedSession` (ny session + `switchActive`). Test-hook `window.__AI_PLAYGROUND_TEST__` exponeras för deterministisk headless-verifiering (rundresa export→import→DOM).
...
- **Markdown-rendering:** AI-textbubblor (plan-svar, info, återställda `kind:'text'`) renderas via `renderMarkdown()` i `main.js`. Säker: HTML escapes först, fenced ```-block till `<pre><code>`, headings/lists/blockquote/inline-code/bold/italic, länkar tillåts bara för `http(s):`/`mailto:` med `target="_blank" rel="noopener noreferrer nofollow"`. Används aldrig på demo-/användarmeddelanden, aldrig `innerHTML` okontrollerat.
